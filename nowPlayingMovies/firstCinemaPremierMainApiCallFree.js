...
pool.query('SELECT cinema_premier_id FROM now_playing_movies', (error, results) => {
    if (results.rowCount !== 0) {
        results.rows.forEach(function(value) {
            cinemaPremierId.push(value.cinema_premier_id);
        });
    }
                    ...
                    cinemaPremierDataOnShow.raw_genres
                        .forEach(function(genre) {
                            if (genre.toLowerCase().replace(" ", "-") === 'science-fiction') {
                                cinemaMovieGenres.push('sci-fi');
                            } else {
                                cinemaMovieGenres.push(genre.toLowerCase().replace(" ", "-"));
                            }
                        });
